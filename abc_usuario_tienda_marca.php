<?
	...
	if (!empty($usuario)) {
	  $resultado= mysql_query("SELECT * FROM usuario_tienda_marca WHERE clave='$usuario'",$conexion);
	  $row = mysql_fetch_array($resultado);
	}
	 $resMEN= mysql_query("SELECT * FROM menu_tienda_marca",$conexion);
...
<script language="JavaScript">
  function valida() {
    if (document.forma.tienda_marca.value == "") {
     alert("Selecciona la tienda.");
	 document.forma.tienda_marca.focus();
     return;
    }
    ...
    document.forma.action='graba_usuario_tienda_marca.php';
	...
	<div class="main">
      <form action="" method="post" name="forma" id="forma">
        ...
        <table width="100%" border="0" align="center" cellpadding="3" cellspacing="0" class="texto">
          ...
          <tr>
            <td><div align="right">Tienda de Marca:
              </div>
              ...
              <select name="tienda_marca" class="campo" id="tienda_marca">
                ...
                <?
					...
					while ($rowTIE = mysql_fetch_array($resultadoTIE)) {
					  ...
					  if ($rowTIE['clave']==$row['tienda_marca']) echo 'selected';
              ...
              <tr>
                <td><span class="rotulo">
                  <select name="cat_opciones" size="<?= $cant_menu; ?>" class="campo" id="cat_opciones" ondblclick="agregaOp(document.forma,this.options[this.selectedIndex].text,this.value);">
                    <?  $resMEN= mysql_query("SELECT * FROM menu_tienda_marca ORDER BY opcion",$conexion);
                ...
                <td><select name="lista_opciones" size="<?= $cant_menu; ?>" class="campo" id="lista_opciones" ondblclick="eliminaOp(document.forma,this.selectedIndex);">
                    <?  $op=explode(',',$row['opciones']);
						for ($i=0; $i<=count($op)-2; $i++) {
						  $claveop=trim($op[$i]);
						  $resMEN= mysql_query("SELECT * FROM menu_tienda_marca WHERE clave=$claveop",$conexion);
