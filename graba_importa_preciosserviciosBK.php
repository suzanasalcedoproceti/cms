	...
	if ($subido) {  // si se subió el archivo
		...
		while(! feof($fn))  {
			...
			if(!isset($linea_error[$le])){
				...
				//Validar Servicio
				if(!preg_match('/^[A-Za-záéíóúÁÉÍÓÚ\s]{1}+[0-9]{1}+$/', trim($lin[2])))
				{
					$error_data.='Columna Subtipo Producto no valido. ';
				}
				//Validar Tipo producto
				if(!preg_match('/^[A-Za-záéíóúÁÉÍÓÚ\s]+$/', trim($lin[3])))
				{
		...
		{
		$resultado = mysql_query("DELETE FROM temp_precioservicios",$conexion);
		$resultado = mysql_query("SELECT 1 FROM temp_precioservicios",$conexion);
