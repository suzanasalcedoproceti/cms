        ...
        <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0" class="texto">
          ...
          <tr>
            ...
            <td colspan="10"><table width="35%" border="0" cellpadding="00">
              ...
              <? 
			     $modelo = $row['modelo'];
			     $query = "SELECT planta.*, existencia.existencia FROM planta LEFT JOIN existencia ON planta.planta = existencia.cedis AND planta.loc = existencia.loc AND producto = '$modelo' ORDER BY cedis, loc";
			     $resultadoEX = mysql_query($query,$conexion);
			     //echo $query;
			  ...
			  ?>
              <tr bgcolor="#FFFFFF">
                <td align="center"><?=$rowEX['planta'];?></td>
                <td align="center"><?=$rowEX['loc'];?></td>
                <td align="center"><?=(int)$rowEX['existencia'];?></td>
              </tr>
              <? } ?>
            </table></td>
          </tr>
...
<label>  (Si es solo para sitios de marca, no se actualizan los precios en la importaci&oacute;n; el precio debe estar en cero para que no aparezcan en sitios de marca</label></td>
        ...
        <!-- Inicia REQ0136 Reglas de producto Daniel Garza no se ha pasado a producción-->
          <tr>
            <td align="right"><strong>Permitir venta sin inventario</strong></td>
            <td colspan="10" align="left"> <input name="venta_sin_inventario" type="checkbox" id="venta_sin_inventario" value="1" <? if($row['venta_sin_inventario']) echo 'checked';?> /> &nbsp;&nbsp;(Solamente aplica para productos que lleven estatus 30)
           </td>
          </tr>
      	<!-- Termina REQ0136 Reglas de producto Daniel Garza no se ha pasado a producción-->
          <tr>
