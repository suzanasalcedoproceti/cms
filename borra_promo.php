...
<div id="container">
    ...
    <form action="lista_promo.php" method="post" name="brinca" id="brinca">
	...
	<div class="main">
      <p>
		...
		<?
                ...
                if (!empty($promo) AND empty($autorizar)) {  // si no está en proceso de autorización

                    $resultado= mysql_query("DELETE FROM promo WHERE clave = '$promo'" ,$conexion);
                      $totalRegistros = mysql_affected_rows() ;
                      if ($totalRegistros > 0) {
                        $mensaje='Registro eliminado.';
						
						@unlink('images/cms/promos/'.$promo.'.jpg');
				...
				elseif (!empty($promo) AND !empty($autorizar))  {  // si está en proceso de autorización
					...
					$original=$row['original'];
           		    ...
           		    $resultado= mysql_query("UPDATE promo SET estatus=1 WHERE clave='$original'",$conexion);
