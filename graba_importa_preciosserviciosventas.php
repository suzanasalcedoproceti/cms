<?
	...
	$nombrearchivo = './imp_oc/archivo_preciosserviciosventas.csv';
	...
	if ($subido) {  // si se subió el archivo
		...
		$direc = $rowCFG['wwwroot']."admin/imp_oc/archivo_preciosserviciosventas.csv";
		...
		{
		$resultado1 = mysql_query("DELETE FROM temp_precioserviciosventas",$conexion);
		$enc3 = mysql_num_rows($resultado1);
		$resultado2 = mysql_query("SELECT 1 FROM temp_precioserviciosventas",$conexion);
		...
		if ($enc>0) 
		  ...
		  $query = "LOAD DATA LOCAL INFILE '$direc' INTO TABLE temp_precioserviciosventas FIELDS TERMINATED BY ',' LINES TERMINATED BY '\r\n' IGNORE 0 LINES"; 
		 
		  $resultado3 = mysql_query($query,$conexion);
		  $error_my = mysql_errno().": ".mysql_error();
	 
		  $resultado4 = mysql_query("SELECT 1 FROM temp_precioserviciosventas",$conexion);
		  ...
		  if ($enc<=0) 
			$error .= 'No se insertaron registros..<br>'.$error_my;
		  	...
		  	$mensaje = 'Se subieron '.$enc.' registros a tabla temporal de precios servicios ventas';
             }
...
//			$resultado = mysql_query("UPDATE cliente SET puntos = 0");
			...
			$query = 'SELECT tps.*,IF(tps.cluster >= 1 and tps.cluster <= 32, tps.cluster,0) as clusterv,tps.subcluster as subclusterv,serv.idservicio as idserviciov, tp.nombre as tipov, stp.subtipo_producto as subtipov FROM temp_precioserviciosventas tps
			...
			while ($row = mysql_fetch_array($resultado)) {
				...
				if($error_data!='')
				{
					...
					$linea_error[$le]=["linea"=>$query,"error"=>$error_data];
				}
				else
				{
					$validos[] = $row;
			...
			if(count($linea_error)==0)
			{
				...
				foreach ($validos as $row) {
					...
					$precio = $row['precio'];
					$costo = $row['costo'];
					$query_val = "SELECT idPrecioservicio FROM precioservicios where cluster=$cluster and subcluster=$subcluster and subtipo_producto='$subtipo_producto'
					...
					$resultadoval = mysql_query($query_val,$conexion);
		  			...
		  			if ($encval<=0){ 
						...
						$query_ins = "INSERT INTO precioservicios(cluster,subcluster,tipo_producto,subtipo_producto,idservicio,costo)
						VALUES ($cluster,$subcluster,'$tipo_producto','$subtipo_producto',$idservicio,$costo)";
						...
						$query_upd = "UPDATE precioservicios SET cluster=$cluster, subcluster=$subcluster, subtipo_producto='$subtipo_producto', idServicio=$idservicio, tipo_producto='$tipo_producto',costo='$costo' WHERE idPrecioservicio=$idPrecioservicio";
