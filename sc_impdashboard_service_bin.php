<?php
	...
	if ($_SERVER['HTTP_HOST']=='localhost' ) {
		$nombrearchivo = './imp_dashboard/tawotd_service.txt';
		$nombrearchivofull = 'd:/www/whirlpool/admin/imp_dashboard/tawotd_service.txt';
		...
		$nombrearchivo = './imp_dashboard/tawotd_service.txt';
		$nombrearchivofull = 'd:/inetpub/wwwroot/admin/imp_dashboard/tawotd_service.txt';
...
//   	$base="wp_test";
	...
	$mensaje = 'Se inició proceso de importación de datos (SAP) dashboard SERVICE'.$CR;
	...
	if ($subido) {  // si se subió el archivo
		...
		$direc = $rowCFG['wwwroot']."admin/imp_dashboard/tawotd_service.txt";
		...
		if ($enc>0) 
		 ...
		 // echo "<br>".$query."<br>";
		  ...
		  $direc = $rowCFG['wwwroot']."admin/imp_dashboard/tawotd_service.txt";
		  $direc_bk = $rowCFG['wwwroot']."admin/imp_dashboard/backup/tawotd_service.txt";
		  ...
		  if ($enc<=0) 
			$error .= 'No se insertaron registros..'.$CR.$error_my.$CR;
		  	...
		  	$mensaje = $CR.$CR.'Se subieron '.$enc.' registros a tabla temporal de datos dashboard SERVICE';
			...
			while ($row = mysql_fetch_array($resultado)) {
				...
				$query = "INSERT INTO dashboard ( folio_update, folio_pos, folio_sap, order_date, saty, delivery_block, delivery_block_txt, reason_rejection, reason_rejection2, credit_status, overall_status, 
												  ...
												  tipo_cliente, nombre_cliente, numero_empleado, nombre_empresa, numero_empresa, entrega, forma_pago, estatus_pago, confirmo_pago, estatus_material,
												   estatus_entrega, no_delivery, comentarios, folio_devolucion, adicionales, tipo_inconformidad, estatus_inconformidad, semaforo, estatus_cliente, feedback,
												   proveedor_logistica
												   ) 
												   SELECT 
													  ...
													  '','','','','','','','','','',
													  CASE 
													  WHEN LENGTH(tracking_number)=9 AND LEFT(tracking_number,2)='17' THEN 'PROMOVIL'
													  WHEN LENGTH(tracking_number)=12 AND LEFT(tracking_number,1)='7' THEN 'FEDEX'
													  WHEN LEFT(tracking_number,3)='CEL' OR LEFT(tracking_number,3)='MON' OR LEFT(tracking_number,3)='THO' THEN 'TRES GUERRAS'
													  WHEN tracking_number LIKE '%CARERA%' THEN 'CARERA'
													  ELSE ''
													  END as proveedor_logistica
												   FROM temp_dashboard
