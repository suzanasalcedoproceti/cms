...
<script language="JavaScript">
  ...
  function borra(id) {
    document.forma.categoria.value = id;
...
<body>
<div id="container">
  <? $tit='Listado de Categor&iacute;as'; include('top.php'); ?>
  <?
   $ver = $_POST['ver'];
   $numpag = $_POST['numpag'];
   $ord = $_POST['ord'];
   ...
   if ($ord=='orden') $orden='ORDER BY orden, nombre';
	...
	<div class="main">
      <form action="" method="post" name="forma" id="forma">
        <table width="100%" border="0" align="center" cellpadding="7" cellspacing="0" class="texto">
          <tr>
            <td><input name="button" type="submit" class="boton_agregar" id="button" value="Agregar categor&iacute;a nueva" onClick="document.forma.action='abc_categoria.php'; document.forma.submit();" /></td>
            <td align="right">&nbsp;</td>
          </tr>
          <tr>
            <td bgcolor="#BBBBBB"><?			
                  ...
                  // y establecer algunas variables
                     ...
                     // construir la condición de búsqueda
                       ...
                       $resultadotot= mysql_query("SELECT * FROM categoria $condicion",$conexion);
                       $totres = mysql_num_rows ($resultadotot);
                       $totpags = ceil($totres/$ver);
                       if ($totres==0)
                          $numpag = 0;
						  
					 echo 'Total de categor&iacute;as en la lista: <b>'.$totres.'</b>';
           
			
			  ?>            </td>
            <td align="right" bgcolor="#BBBBBB"><input name="categoria" type="hidden" id="categoria" />
        ...
        <table width="100%" border="0" align="center" cellpadding="2" cellspacing="3">
          ...
          <tr class="texto" bgcolor="#F4F4F2">
            ...
            <td nowrap="nowrap" bgcolor="<? if($ord=='nombre') echo '#DDDDDD'; ?>"><b><a href="javascript:ordena('nombre');" class="texto">Categor&iacute;a <img src="images/orden.png" width="14" height="15" border="0" align="absmiddle" /></a></b></td>
            <td><div align="center"><strong>M&iacute;nimo venta</strong></div></td>
            <td><div align="center"><b>Acci&oacute;n</b></div></td>
          </tr>
          <?

             $resultado= mysql_query("SELECT * FROM categoria $condicion $orden LIMIT $regini,$ver",$conexion);
             while ($row = mysql_fetch_array($resultado)){ 

				$categoria= $row['clave'];
			    $resPRO= mysql_query("SELECT 1 FROM producto WHERE categoria=$categoria",$conexion);
			    $rel = mysql_num_rows($resPRO);
			    $resSCAT= mysql_query("SELECT 1 FROM subcategoria WHERE categoria=$categoria",$conexion);
			    $rel += mysql_num_rows($resSCAT);
          ...
          <tr class="texto" valign="top" bgcolor="<? if ($color) echo '#FFFFFF'; else echo '#F4F4F2'; ?>">
            ...
            <td align="center" nowrap="nowrap" bgcolor="#FFFFFF"><?= ($row['minimo_venta']) ? 'SI' : '';?></td>
            <td align="center" nowrap="nowrap" bgcolor="#FFFFFF">
               	<? if ($row['estatus']==1) { ?><a href="abc_categoria.php?categoria=<?= $row['clave']; ?>"><img src="images/editar.png" alt="Editar Categoría" width="14" height="16" border="0" align="absmiddle" /></a><? } else  { ?><img src="images/editar_off.png" width="14" height="16" align="absmiddle" /><? } ?>
       	  		<? if ($rel<=0 AND op_aut($modulo) AND $row['estatus']==1 && !$row['accesorios'] && !$row['garantias']) { ?><a onclick="return confirm('\u00bfEst&aacute;s seguro que deseas\nBorrar la Categor&iacute;a?')" href="javascript:borra('<?= $row['clave']; ?>');"><img src="images/borrar.png" alt="Borrar Categoría" width="14" height="15" border="0" align="absmiddle" /></a><? } else  { ?><img src="images/borrar_off.png" width="14" height="15" align="absmiddle" /><? } ?>            </td>
		  </tr>
          <?
                 } // WHILE
                 mysql_close();
