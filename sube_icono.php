...
if (1 || !empty($documento)) {
	...
	$size=30720;  // tamaño máximo en Kb
	...
	$errores[2]='*El archivo excede '.$MAX_FILE_SIZE.'.';  // excede el parámetro MAX_FILE_SIZE en la form
	$errores[3]='*El archivo se subió parcialmente.';
	...
	if ($_FILES[$doc_valor]['name']!="") {  // si hay imagen a subir
	  ...
	  if ($_FILES[$doc_valor]['error']>0) { $error.=$errores[$_FILES[$doc_valor]['error']].'<br>'; }
		...
		if ($_FILES[$doc_valor]['type']!="image/gif" && $_FILES[$doc_valor]['type']!="image/gif") { $error.='S&oacute;lo se pueden subir archivos GIF.<br>'; } 
