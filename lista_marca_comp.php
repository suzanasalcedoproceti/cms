<?
   ...
   include('../conexion.php');
	...
	if ($_POST['accion']=='borrar') {
		$marca=$_POST['marca']+0;
        $resultado= mysql_query("DELETE FROM comp_marca WHERE clave = $marca" ,$conexion);
	}
?>
...
<script language="JavaScript">
  function ir(form, Pag) {
    ...
    form.action='lista_marca_comp.php';
    form.submit();
  }
  function ordena(orden) {
    document.forma.ord.value = orden;
	document.forma.numpag.value = 1;
    document.forma.action='lista_marca_comp.php';
    document.forma.submit();
  }
  function borra(id) {
    document.forma.marca.value = id;
...
<body>
<div id="container">
  <? $tit='Listado de Marcas de la Competencia'; include('top.php'); ?>
  <?
   $ver = $_POST['ver'];
   $numpag = $_POST['numpag'];
   $ord = $_POST['ord'];
	...
	<div class="main">
      <form action="" method="post" name="forma" id="forma">
        <table width="100%" border="0" align="center" cellpadding="7" cellspacing="0" class="texto">
          <tr>
            <td><input name="button" type="submit" class="boton_agregar" id="button" value="Agregar marca nueva" onClick="document.forma.action='abc_marca_comp.php'; document.forma.submit();" /></td>
            <td align="right">&nbsp;</td>
          </tr>
          <tr>
            <td bgcolor="#BBBBBB"><?
                  ...
                  // y establecer algunas variables
                     ...
                     // construir la condición de búsqueda
                       ...
                       $resultadotot= mysql_query("SELECT * FROM comp_marca $condicion",$conexion);
                       $totres = mysql_num_rows ($resultadotot);
                       $totpags = ceil($totres/$ver);
                       if ($totres==0)
                          $numpag = 0;
						  
					 echo 'Total de marcas en la lista: <b>'.$totres.'</b>';
			
			  ?>            </td>
            <td align="right" bgcolor="#BBBBBB"><input name="marca" type="hidden" id="marca" />
        ...
        <table width="100%" border="0" align="center" cellpadding="2" cellspacing="3">
          ...
          <tr class="texto" bgcolor="#F4F4F2">
            <td nowrap="nowrap" bgcolor="<? if($ord=='nombre') echo '#DDDDDD'; ?>"><b><a href="javascript:ordena('nombre');" class="texto">Marca <img src="images/orden.png" width="14" height="15" border="0" align="absmiddle" /></a></b></td>
            <td><div align="center"><b>Acci&oacute;n</b></div></td>
          </tr>
          <?

             $resultado= mysql_query("SELECT * FROM comp_marca $condicion $orden LIMIT $regini,$ver",$conexion);
             while ($row = mysql_fetch_array($resultado)){ 

				$marca= $row['clave'];
			    $resPRO= mysql_query("SELECT 1 FROM comp_producto WHERE marca=$marca",$conexion);
          ...
          <tr class="texto" valign="top" bgcolor="<? if ($color) echo '#FFFFFF'; else echo '#F4F4F2'; ?>">
            ...
            <td align="center" nowrap="nowrap" bgcolor="#FFFFFF">
               	<a href="abc_marca_comp.php?marca=<?= $row['clave']; ?>"><img src="images/editar.png" alt="Editar Marca" width="14" height="16" border="0" align="absmiddle" /></a>
       	  		<? if ($rel<=0) { ?><a onclick="return confirm('¿Estás seguro que deseas\nBorrar la Marca?')" href="javascript:borra('<?= $row['clave']; ?>');"><img src="images/borrar.png" alt="Borrar Marca" width="14" height="15" border="0" align="absmiddle" /></a><? } else  { ?><img src="images/borrar_off.png" width="14" height="15" align="absmiddle" /><? } ?>            </td>
		  </tr>
          <? } // WHILE ?>
        </table>
        <table width="100%" border="0" align="center" cellpadding="7" cellspacing="0" class="texto">
