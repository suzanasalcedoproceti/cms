...
$query = "SELECT * FROM pedido WHERE folio = $pedido AND estatus = '0'";
		...
		if ($enc > 0) {
			...
			if ($reg <= 0) { 
				...
				$resultado= mysql_query($query,$conexion);
				//print_r($row);
				$data = array('folio' => $pedido, 'codigo_seguridad'=> $row['codigo_seguridad']);
				$data = json_encode($data);
				$folio_pedido = $pedido;
				//echo $data;
				$optional_headers = "Content-type: application/json";
				$response = json_decode(do_post_request('https://pos.tiendawhirlpool.com/service/odc', $data, $optional_headers));
				//print_r($response);
				/*
