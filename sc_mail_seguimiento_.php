...
$sql = "     
SELECT 
    p.folio AS folio_pedido,COUNT(p.folio) as p_pedidos,
    IFNULL((SELECT count(ds.folio_pedido) 
            FROM dashboard ds 
            WHERE ds.folio_pedido = p.folio AND ds.sufijo<>'G' AND ds.entrega = 'domicilio'
            GROUP BY ds.folio_pedido),
    0) AS ds_pedidos,
    IF ( MOD(DATEDIFF(CURDATE(),p.fecha),10) = 0 AND DATEDIFF(CURDATE(),p.fecha) > 0, 1, 0) AS  seguimiento,
    IFNULL((SELECT COUNT(ds.folio_pedido) 
            FROM dashboard ds 
            WHERE ds.folio_pedido = p.folio AND ds.sufijo <> 'G' AND ds.guia <> '' AND ds.entrega = 'domicilio' and billing_doc <> ''
            GROUP BY ds.folio_pedido),
    0) AS ds_pedidos_entregados,
    IFNULL((SELECT sum(ps.productos_guia) 
            FROM pedido_seguimiento ps 
            WHERE ps.folio_pedido = p.folio AND ps.tipo in ('Entrega Parcial')  
            GROUP BY ps.folio_pedido),
    0) AS ps_total,
    p.*,
    p.total AS total_del_pedido,
    pedido_netpay.*
  FROM pedido p 
  INNER JOIN detalle_pedido dp ON p.folio = dp.pedido AND dp.es_flete=0 AND dp.es_garantia = 0 AND LOWER(dp.tipo_entrega)='domicilio'
  LEFT JOIN pedido_netpay ON pedido_netpay.folio = p.folio
  WHERE p.estatus = 1 AND p.fecha >= '2017-02-01' and p.tipo_venta<>'PR' and p.tienda<>29
  and dp.sustituido=0
  and p.folio not in (SELECT folio_refacturacion from pedido)
  and  concat(dp.partida,LPAD(dp.pedido,9,'0')) not in (SELECT concat(detalle_pedido.partida,LPAD(detalle_pedido.pedido,9,'0')) FROM `detalle_pedido` inner JOIN dashboard on detalle_pedido.pedido=dashboard.folio_pedido and detalle_pedido.modelo=dashboard.material where dashboard.reason_rejection<>'' OR LEFT(dashboard.planta,2)='FM'  group by detalle_pedido.pedido,detalle_pedido.partida)  
  GROUP BY p.folio
  HAVING  ( 
...
// script para generar y enviar correo cuando se graba el pedido, o cuando se procesa en CMS
  ...
  if ($row['fdp_tdc']>0) $txt_formas_pago .= "Tarjeta de Cr&eacute;dito: <strong>".number_format($row['fdp_tdc'],2)."</strong> <br>";
    ...
    if($row['seguimiento'] == 1  && ($row['ds_pedidos_entregados'] == 0 || (($row['ds_pedidos_entregados'] == $row['ps_total']) && ($row['ds_pedidos_entregados'] < $row['ds_pedidos'])))) {
      // seguimiento
    ...
    }elseif( $row['ds_pedidos_entregados'] > $row['ps_total'] ){
      // entrega o env&iacute;o parcial
	 $query = "SELECT  proveedor_logistica, guia  FROM dashboard AS ds WHERE ds.guia<>'' AND billing_doc<>'' AND ds.sufijo <> 'G' and ds.folio_pedido=$folio_pedido and guia not in (select dp.guia from pedido_seguimiento dp where dp.folio_pedido = $folio_pedido)";
    ...
    while ($rowsDP = mysql_fetch_array($resDP)) {
      ...
      $texto_mensaje_correo = 'Los siguientes productos han sido programados para entrega en los próximos 10 días a través de:<br>'.$texto_guias;
...
//echo 'paso n<br>';
    ...
    $query = "SELECT detalle_pedido.*, marca.nombre AS nombre_marca, pedido.origen, pedido.tipo_venta,
                  IFNULL((SELECT  IF(ds.guia = 'No Aplica','', IF(ds.guia='','',ds.guia)) as ds_guia FROM dashboard AS ds WHERE ds.material = detalle_pedido.modelo and ds.folio_pedido=$folio_pedido LIMIT 1),'') as guia
                 FROM detalle_pedido 
                 LEFT JOIN marca ON detalle_pedido.marca = marca.clave
                 LEFT JOIN pedido ON detalle_pedido.pedido = pedido.folio
                 WHERE pedido = $folio_pedido AND detalle_pedido.es_flete=0 AND detalle_pedido.es_garantia = 0 AND LOWER(detalle_pedido.tipo_entrega)='domicilio' and 
                 IFNULL((SELECT  IF(ds.guia = 'No Aplica','No Aplica', IF(ds.guia='','',ds.guia)) as ds_guia FROM dashboard AS ds WHERE ds.material = detalle_pedido.modelo and ds.folio_pedido=$folio_pedido LIMIT 1),'')!='' AND billing_doc<>'' AND detalle_pedido.partida not in (SELECT ps.partida FROM pedido_seguimiento ps where ps.folio_pedido=$folio_pedido) ORDER BY partida";
    if($correo_tipo == 'Seguimiento')
    {
    // obtener html de cada partida
    $query = "SELECT detalle_pedido.*, marca.nombre AS nombre_marca, pedido.origen, pedido.tipo_venta,
                  IFNULL((SELECT  IF(ds.guia = 'No Aplica','No Aplica', IF(ds.guia='','',ds.guia)) as ds_guia FROM dashboard AS ds WHERE ds.material = detalle_pedido.modelo and ds.folio_pedido=$folio_pedido LIMIT 1),'') as guia
                 FROM detalle_pedido 
                 LEFT JOIN marca ON detalle_pedido.marca = marca.clave
                 LEFT JOIN pedido ON detalle_pedido.pedido = pedido.folio
                 WHERE pedido = $folio_pedido AND detalle_pedido.es_flete=0 AND detalle_pedido.es_garantia = 0 AND LOWER(detalle_pedido.tipo_entrega)='domicilio' and 
                 IFNULL((SELECT  IF(ds.guia = 'No Aplica','No Aplica', IF(ds.guia='','',ds.guia)) as ds_guia FROM dashboard AS ds WHERE ds.material = detalle_pedido.modelo and ds.folio_pedido=$folio_pedido LIMIT 1),'')='' ORDER BY partida";
				 //echo "<br>".$query;
    }
...
';
    ...
    if($correo_tipo=='Entrega Parcial')
    {
      $resultadoDG = mysql_query($query,$conexion);
