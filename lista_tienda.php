<?
   ...
   if ($_POST['accion'] == 'borrar') {
   		$resultado = mysql_query("DELETE FROM tienda WHERE clave = $tienda LIMIT 1",$conexion);
	    $resultado = mysql_query("DELETE FROM  marca_omitida WHERE tienda = $tienda",$conexion);
   }
?>
...
<script language="JavaScript">
  ...
  function borra(id) {
    document.forma.tienda.value = id;
...
<body>
<div id="container">
  <? $tit='Listado de Tiendas'; include('top.php'); ?>
	<div class="main">
      <form action="" method="post" name="forma" id="forma">
        <table width="100%" border="0" align="center" cellpadding="7" cellspacing="0" class="texto">
          <tr>
            <td><input name="button" type="submit" class="boton_agregar" id="button" value="Agregar tienda nueva" onClick="document.forma.action='abc_tienda.php'; document.forma.submit();" /></td>
            <td align="right">&nbsp;</td>
          </tr>
          <tr>
            <td bgcolor="#BBBBBB"><?
                  ...
                  // y establecer algunas variables
                     ...
                     $condicion = "WHERE 1 ";
                     if($_SESSION['usr_service']==1) $condicion .= "AND tienda_service=1 ";



                     // construir la condición de búsqueda

                       $resultadotot= mysql_query("SELECT * FROM tienda $condicion",$conexion);
                       $totres = mysql_num_rows ($resultadotot);
                       $totpags = ceil($totres/$ver);
                       if ($totres==0)
                          $numpag = 0;
						  
					 echo 'Total de tiendas en la lista: <b>'.$totres.'</b>';
			
			  ?>            </td>
            <td align="right" bgcolor="#BBBBBB"><input name="tienda" type="hidden" id="tienda" />
        ...
        <table width="100%" border="0" align="center" cellpadding="2" cellspacing="3">
          ...
          <tr class="texto" bgcolor="#F4F4F2">
            <td nowrap="nowrap" bgcolor="<? if($ord=='orden') echo '#DDDDDD'; ?>"><b><a href="javascript:ordena('orden');" class="texto">Tienda   <img src="images/orden.png" width="14" height="15" border="0" align="absmiddle" /></a></b></td>
            <td nowrap="nowrap"><b>Login</b></td>
            <td nowrap="nowrap"><b>Activa</b></td>
            <td><div align="center"><b>Acci&oacute;n</b></div></td>
          </tr>
          <?
             $resultado= mysql_query("SELECT * FROM tienda $condicion $orden LIMIT $regini,$ver",$conexion);
             while ($row = mysql_fetch_array($resultado)){ 

				$tienda= $row['clave'];
			    $resUSR= mysql_query("SELECT 1 FROM usuario_tienda WHERE tienda=$tienda",$conexion);
			    $rel = mysql_num_rows($resUSR);
          ...
          <tr class="texto" valign="top" bgcolor="<? if ($color) echo '#FFFFFF'; else echo '#F4F4F2'; ?>">
            ...
            <td bgcolor="#FFFFFF"><?= ($row['activa']) ? 'SI' : 'NO'; ?></td>
            <td align="center" nowrap="nowrap" bgcolor="#FFFFFF">
              <a href="abc_tienda.php?tienda=<?= $row['clave']; ?>"><img src="images/editar.png" alt="Editar Tienda" width="14" height="16" border="0" align="absmiddle" /></a>
     	  	  <? if ($rel<=0) { ?><a onclick="return confirm('¿Estás seguro que deseas\nBorrar la Tienda?')" href="javascript:borra('<?= $row['clave']; ?>');"><img src="images/borrar.png" alt="Borrar Tienda" width="14" height="15" border="0" align="absmiddle" /></a><? } else  { ?><img src="images/borrar_off.png" width="14" height="15" align="absmiddle" /><? } ?>            </td>
		  </tr>
          <?
                 } // WHILE
                 mysql_close();
