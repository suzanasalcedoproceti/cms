...
<div class="main">
      <p>
		...
		$query = "SELECT * FROM pedido WHERE folio = $pedido AND estatus = '3'";
		...
		if ($enc > 0) {
			...
			if ($reg <= 0) { 
				...
				if ($envia_mail) {
						...
						$txt_formas_pago= '';
						if ($row['fdp_tdd']>0) $txt_formas_pago = "Tarjeta de Débito: <strong>".number_format($row['fdp_tdd'],2)."</strong> <br>";
