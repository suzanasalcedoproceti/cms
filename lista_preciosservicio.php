...
function save_row(no)
{
 ...
 $("#showedt"+no).hide();   
    ...
    $.post("graba_precioservicioajax.php", { idprecioserv: no,idprecio: idprecio,idtiposervicio:idtiposervicio,tipo_producto:tipo_producto,subtipo_producto:subtipo_producto,cluster:cluster}, function(data)
...
function cancel_row(no)
{
 ...
 $(function() {
    ...
    $("#tipo_productosels").change(function(event) { 
...
<form action="" method="post" name="forma" id="forma">
        <table width="95%" border="0" align="left" cellpadding="7" cellspacing="0" class="texto">
         <tr>
            <td><input name="button" type="submit" class="boton_agregar" id="button" value="Agregar precio servicio" onClick="document.forma.action='abc_precioservicios.php'; document.forma.submit();" /></td>
            <td align="right">&nbsp;</td>
          </tr>

          <tr>
              <td>
                 <table width="90%"  cellpadding="7" cellspacing="0" class="texto">
                     <tr><td>Cluster: <br>
        ...
        ?>            </td>
          ...
          <tr class="texto" valign="top" bgcolor="<? if ($color) echo '#FFFFFF'; else echo '#F4F4F2'; ?>">
              ...
              <div id="tipo_prod<?= $row['idPrecioservicio']; ?>"><?= substr($row['tipo_producto'], 1);?></div>  
                <select name="tipo_producto" class="campo" id="tipo_producto<?= $row['idPrecioservicio']; ?>" style="width: 90px; display: none;">  
                       ...
                       ?>
                            ...
                            <select name="tipo_productosels" class="campo" id="tipo_productosels">
                             <option value="">Cualquier tipo de producto...</option>
                             <?  
                              $resultadoEDO = mysql_query("SELECT * FROM tipo_producto ORDER BY nombre",$conexion);
                              while ($rowEDO = mysql_fetch_array($resultadoEDO)) {
            ...
            <td align="center" nowrap="nowrap" bgcolor="#FFFFFF">
              <input type="button" id="edit_button<?= $row['idPrecioservicio']; ?>" value="Editar" class="edit" onclick="edit_row(<?= $row['idPrecioservicio'];?>)"> 
